...
class DataRecorderNode(object):
    ...
    def try_record(self):
        if self.pose is None or self.gripper is None or \
           self.eye_image is None or self.agent_image is None or self.ext_force is None:
            return False

        if self.pose is None or not self.new_gripper or \
        self.eye_image is None or self.agent_image is None or not self.new_ext_force:
            return False
    ...
    def convert_to_dataset_format(self):
        ...
        ext_force = np.zeros(frame_count)
        view1_frames = []
        view2_frames = []
        ...
        return eef_position, eef_orientation, gripper_width, ext_force, np.array(view1_frames), np.array(view2_frames)
    ...
    try:
        ...
        while episode_index < args.num_episodes and not recorder.exit_all:
            ...
            with tqdm(total=num_frames, desc=f"Ep {episode_index:03d} Frame", ncols=80) as pbar:
                while len(recorder.records) < num_frames:
                    if recorder.active == 2:
                        ...
                        elif recorder.eye_image is None or recorder.agent_image is None:
                            pass
            ...
            # 如果有錄到數據 (正常完成或按右鍵)
                ...
                eef_position, eef_orientation, gripper_width, ext_force, view1_frames, view2_frames = recorder.convert_to_dataset_format()

                if view1_frames is not None and len(view1_frames) > 0:
                    try:
                        dataset.add_episode(
                            episode_index, eef_position, eef_orientation, gripper_width, ext_force,
